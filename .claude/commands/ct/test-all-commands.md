...
# Test All Commands Command

Tests all commands in `.claude/commands/` using parallel subagents with multiple iterations.
...
This command:
1. Discovers all command files in `.claude/commands/` dynamically
2. Spawns parallel subagents (one per command)
3. Each subagent tests the command using `testing-commands-with-subagents`
4. Each writes an evaluation summary to root directory
5. Repeats for multiple iterations to refine results

**Focus:** Determine if commands enforce ALL steps under pressure scenarios
...
## Step 1: Discover Commands Dynamically

Find all command files and count them:
...
Store command list, count, and iterations for all subsequent steps.
...
Repeat Step 4 for total iterations specified (default 3, or user-specified value).
